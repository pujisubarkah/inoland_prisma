...
const fetchData = async () => {
  ...
  try {
    ...
    users.value = []
    roles.value = [] // <-- pastikan ini array, bukan undefined
  } finally {
...
<template>
  <div class="w-full mt-24">
    ...
    <div class="w-11/12 mx-auto mb-6 flex justify-between items-center">
      <div></div> <!-- Spacer kiri, bisa kosong -->
      <button
    ...
    <table
      class="border-collapse table-auto w-11/12 text-sm shadow-xl rounded-xl overflow-hidden bg-white ml-20"
    >
      <thead>
      ...
      <tbody>
        <tr v-for="(user, index) in currentItems" :key="user.id" class="hover:bg-blue-50 transition">
          ...
          <td class="border-b p-4 font-bold text-blue-700">
            {{ (roles.value ?? []).find(r => r.id === user.role_id)?.role_name || '-' }}
          </td>
          <td class="border-b p-4 font-bold text-blue-700">
        ...
        <tr v-if="currentItems.length === 0">
          <td colspan="7" class="text-center p-4 text-blue-700">Tidak ada pengguna ditemukan.</td>
        </tr>
      </tbody>
    </table>
    ...
    <!-- Modal Edit Role -->
    <div v-if="editUser" class="fixed inset-0 z-50 flex items-center justify-center">
      <!-- Overlay transparan dan blur -->
      <div class="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>
      <div class="relative bg-white p-8 rounded-xl shadow-2xl w-[400px] z-10">
        <h3 class="text-xl font-bold mb-4 text-blue-700 text-center">
          Edit Role & Verifikasi untuk <span class="text-cyan-600">{{ editUser.nama_lengkap }}</span>
        </h3>
        ...
        <div class="mb-6">
          <label class="font-semibold text-blue-700 mb-2 block">Status Verifikasi</label>
          <select v-model="selectedVerified" class="w-full p-3 border border-blue-300 rounded-lg text-blue-700 font-semibold focus:outline-none focus:ring-2 focus:ring-cyan-400">
    ...
    <div v-if="isAddModalOpen" class="fixed inset-0 z-50 flex items-center justify-center">
      <div class="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>
      <div class="relative bg-white p-8 rounded-xl shadow-2xl w-[600px] z-10">
        <h3 class="text-xl font-bold mb-4 text-blue-700 text-center">Tambah Pengguna Baru</h3>
        <form @submit.prevent="handleAddUser">
          <div class="mb-4 flex items-center gap-4">
            <label for="add-fullname" class="form-label">Nama <br> Lengkap</label>
            <input id="add-fullname" v-model="addForm.nama_lengkap" type="text" required class="form-input" />
          </div>
          <div class="mb-4 flex items-center gap-4">
            <label for="add-username" class="form-label">Username</label>
            <input id="add-username" v-model="addForm.username" type="text" required class="form-input" />
          </div>
          <div class="mb-4 flex items-center gap-4">
            <label for="add-instansi" class="form-label">Instansi</label>
            <input id="add-instansi" v-model="addForm.instansi" type="text" required class="form-input" />
          </div>
          <div class="mb-4 flex items-center gap-4">
            <label for="add-email" class="form-label">Email</label>
            <input id="add-email" v-model="addForm.email" type="email" required class="form-input" />
          </div>
          <div class="mb-4 flex items-center gap-4">
            <label for="add-password" class="form-label">Password</label>
            <input id="add-password" v-model="addForm.password" type="password" required class="form-input" />
          </div>
          <div class="mb-4 flex items-center gap-4">
            <label for="add-confirm" class="form-label leading-tight">
              Konfirmasi<br>Password
            </label>
            <input id="add-confirm" v-model="addForm.confirm" type="password" required class="form-input" />
          </div>
          <div class="flex justify-end gap-2 mt-6">
            <button type="button" @click="isAddModalOpen = false" class="bg-red-500 text-white py-2 px-5 rounded-lg font-bold shadow hover:bg-red-600 transition">Batal</button>
            <button type="submit" class="bg-gradient-to-r from-blue-700 via-blue-400 to-cyan-400 text-white py-2 px-5 rounded-lg font-bold shadow hover:scale-105 hover:shadow-xl transition-all">Simpan</button>
          </div>
        </form>
      </div>
    </div>
