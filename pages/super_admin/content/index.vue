<template>
  <div class="w-full mt-24">
    <h2 class="text-3xl font-extrabold text-center mb-6">Daftar Berita</h2>
      ...
      <table class="border-collapse table-auto w-11/12 text-sm shadow-xl rounded-xl overflow-hidden bg-white">
        ...
        <tbody>
          <tr v-for="berita in paginatedBerita" :key="berita.id" class="hover:bg-blue-50 transition">
            <td class="border-b p-4 text-gray-700 font-semibold align-top">{{ formatDate(berita.date) }}</td>
            <td class="border-b p-4 text-gray-800 font-bold align-top">{{ berita.title }}</td>
            <td class="border-b p-4 text-gray-700 align-top w-96">
              <div class="whitespace-pre-line">{{ berita.deskripsi }}</div>
            </td>
            <td class="border-b p-4 text-gray-700 align-top w-40">
              <a :href="berita.imageUrl" target="_blank" class="text-cyan-600 underline hover:text-cyan-700 block truncate">
                {{ berita.imageUrl }}
              </a>
            </td>
    ...
    <div class="flex flex-col items-center mt-8 gap-2">
      <span class="text-gray-700 font-semibold">Halaman {{ currentPage }} dari {{ totalPages }}</span>
    ...
    <!-- Modal Tambah Berita (neutral styling) -->
    <div
      v-if="showModal"
    ...
    >
      <div class="bg-white rounded-xl shadow-xl p-8 max-w-lg w-full mx-auto relative">
        <button
          ...
          class="absolute top-4 right-4 text-gray-700 text-2xl font-bold hover:text-red-500 transition"
        >×</button>
        <h2 class="text-2xl font-extrabold text-center text-gray-800 mb-6">Tambah Berita Baru</h2>
        <form @submit.prevent="handleSubmit" class="space-y-5">
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Tanggal</label>
            <input v-model="formData.date" type="date" required class="w-full rounded-lg px-4 py-2 border border-gray-300 bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-200 shadow" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Judul</label>
            <input v-model="formData.title" type="text" required placeholder="Judul berita" class="w-full rounded-lg px-4 py-2 border border-gray-300 bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-200 shadow" />
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Deskripsi</label>
            <textarea v-model="formData.deskripsi" rows="5" required placeholder="Deskripsi berita" class="w-full rounded-lg px-4 py-2 border border-gray-300 bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-200 shadow"></textarea>
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-700 mb-2">Upload Gambar</label>
            <input type="file" accept="image/*" @change="handleImageUpload" required class="w-full rounded-lg px-4 py-2 border border-gray-300 bg-white text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-200 shadow" />
          ...
          <div class="flex justify-end gap-3 mt-6">
            <button type="button" @click="showModal = false" class="bg-gray-300 text-gray-800 px-5 py-2 rounded-lg font-medium shadow hover:bg-gray-400 transition">Batal</button>
            <button type="submit" :disabled="isUploading" class="bg-gray-800 text-white px-5 py-2 rounded-lg font-semibold shadow hover:bg-gray-900 transition-all disabled:opacity-60">
