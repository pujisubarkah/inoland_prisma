<template>
  <div class="p-6">
    <h2 class="text-2xl font-semibold mb-2">Inovasi / Pengajuan</h2>
    <div class="w-full mt-24">
      <h1 class="text-3xl font-extrabold text-center font-poppins my-5">Tambah Inovasi</h1>
      ...
      <div class="w-full flex justify-center mt-16">
        <table class="border-collapse table-auto w-11/12 text-sm shadow-xl rounded-xl overflow-hidden bg-white">
          ...
          <tbody>
            ...
            >
              <td class="border-b p-4 font-semibold text-gray-700">{{ item.tahun }}</td>
              <td class="border-b p-4 font-semibold text-gray-800">{{ item.judul_inovasi }}</td>
              <td class="border-b p-4 font-semibold text-gray-700">{{ item.kld }}</td>
              <td class="border-b p-4 font-semibold text-gray-700">{{ item.inovator || '-' }}</td>
              <td class="border-b p-4 font-semibold text-gray-700">{{ item.sdgs || '-' }}</td>
              ...
              <td class="border-b p-4 font-semibold flex gap-2 w-32">
                <button @click="handleEditClick(item.id)" class="text-gray-700 hover:text-gray-900 bg-white border border-gray-200 rounded-lg px-2 py-1 shadow hover:scale-110 transition">
                  ✏️
                </button>
      ...
      >
        <div
          class="bg-white rounded-xl shadow-xl p-8 max-w-lg w-full mx-auto relative overflow-y-auto"
        ...
        >
          ...
          >×</button>
          <h2 class="text-2xl font-extrabold text-center text-gray-800 mb-6">Tambah Inovasi Baru</h2>
          <form @submit.prevent="handleAddInovasi" class="space-y-5">
            <div>
              ...
              <input v-model="formInovasi.tahun" type="text" required placeholder="Tahun"
                class="w-full rounded-lg px-4 py-2 border border-gray-300 bg-white text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-200 shadow" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">Judul Inovasi</label>
              <input v-model="formInovasi.judul_inovasi" type="text" required placeholder="Judul Inovasi"
                class="w-full rounded-lg px-4 py-2 border border-gray-300 bg-white text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-200 shadow" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">KLD</label>
              <input v-model="formInovasi.kld" type="text" required placeholder="KLD"
                class="w-full rounded-lg px-4 py-2 border border-gray-300 bg-white text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-200 shadow" />
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">Inovator</label>
            ...
            <div>
              ...
              <textarea v-model="formInovasi.deskripsi" rows="5" required placeholder="Deskripsi inovasi"
                class="w-full rounded-lg px-4 py-2 border border-gray-300 bg-white text-gray-800 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-200 shadow"></textarea>
            </div>
            <div class="flex justify-end gap-3 mt-6">
              <button type="button" @click="onClose"
                class="bg-gray-300 text-gray-800 px-5 py-2 rounded-lg font-medium shadow hover:bg-gray-400 transition">
                Batal
              </button>
              <button type="submit"
                class="bg-gray-800 text-white px-5 py-2 rounded-lg font-semibold shadow hover:bg-gray-900 transition-all">
                Simpan
              </button>
            </div>
          </form>
