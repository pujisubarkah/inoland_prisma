...
const VideoSection = () => {
  ...
  const Quiz: React.FC<QuizProps> = ({ quiz, onQuizComplete }) => {
  ...
  useEffect(() => {
    const savedProgress = localStorage.getItem('quizProgress') ? JSON.parse(localStorage.getItem('quizProgress') as string) : null;
    if (savedProgress) {
  ...
  const handleQuizComplete = (videoIndex: number) => {
  ...
  return (
    <div style={{ display: 'flex'}}>
      ...
      <div style={{ flex: 1, padding: '20px', backgroundColor: '#ecf0f1' }}>
        <section className="video-section p-4 w-full md:w-2/3 mx-auto bg-blue shadow-lg rounded-lg">
